  ...
  return (
    <div style={s.page}>
      <div style={s.container}>
        ...
        <div style={s.grid4}>
          {[
            { label: "Balance", value: fmt(Math.abs(balance)), sub: balance >= 0 ? "positive" : "overspent", color: balance >= 0 ? T.color.income : T.color.expense, icon: "💼" },
            { label: "Income", value: fmt(income), sub: `${scopedTxs.filter((t) => t.type === "income").length} sources`, color: T.color.income, icon: "⬇️" },
            { label: "Expenses", value: fmt(expenses), sub: `${expenseTxs.length} items`, color: T.color.expense, icon: "⬆️" },
            { label: "Savings rate", value: `${savingsRate}%`, sub: savingsRate >= savingsTarget ? "on track" : "save more", color: T.brand.primary, icon: "📊" },
          ].map((m) => (
            ...
            {pieData.length > 0 && (
              <div style={s.panel}>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    <Pie data={pieData} cx="50%" cy="50%" labelLine={false} label={({ name, value }) => `${name} ${value}%`} outerRadius={70} fill={T.brand.primary} dataKey="value">
                      {pieData.map((_, i) => (
                  ...
                  <div key={t.id} style={s.txRow}>
                    <div style={s.txLeft}>
                      ...
                      <div>
                        ...
                        <div style={s.txType}>
                          {t.type} · {dateStr}
                          {t.is_recurring && <span style={{ color: T.brand.primary, fontWeight: 500, marginLeft: 6 }}>🔄 {getFrequencyLabel(t.frequency)}</span>}
                        </div>
                        {t.is_recurring && nextDate && <div style={{ fontSize: 10, color: T.color.blue, marginTop: 2 }}>Next: {nextDate}</div>}
                      </div>
                    </div>
...
const s = {
  ...
  datePill: { fontSize: 12, color: T.text.secondary, background: T.bg.elevated, border: `1px solid ${T.bg.border}`, borderRadius: 20, padding: "5px 12px" },
  viewToggle: { display: "flex", background: T.bg.base, borderRadius: 8, border: `1px solid ${T.bg.border}`, overflow: "hidden", marginBottom: 12 },
  viewBtn: { padding: "5px 12px", fontSize: 11, fontWeight: 500, border: "none", background: "transparent", color: T.text.secondary, cursor: "pointer", whiteSpace: "nowrap" },
  viewBtnActive: { background: T.brand.primary, color: T.color.white },
  grid4: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 10, marginBottom: "1.25rem" },
  mcard: { background: T.bg.surface, borderRadius: 12, border: `1px solid ${T.bg.border}`, padding: "14px 16px" },
  mLabel: { fontSize: 11, color: T.text.secondary, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 500, marginBottom: 4 },
  mVal: { fontSize: 21, fontWeight: 500, lineHeight: 1 },
  mSub: { fontSize: 11, color: T.text.secondary, marginTop: 5 },
  ...
  panelHd: { fontSize: 11, fontWeight: 500, color: T.text.secondary, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 14, display: "flex", alignItems: "center", gap: 6, justifyContent: "space-between" },
  seg: { display: "flex", background: T.bg.base, borderRadius: 8, border: `1px solid ${T.bg.border}`, overflow: "hidden", marginBottom: 8 },
  ...
  input: { width: "100%", marginBottom: 8, padding: "8px 10px", borderRadius: 8, border: `1px solid ${T.bg.border}`, background: T.bg.base, color: T.color.white, fontSize: 13, outline: "none" },
  ...
  exportBtn: { padding: "4px 10px", fontSize: 11, fontWeight: 500, border: `1px solid ${T.bg.border}`, borderRadius: 6, background: "transparent", color: T.text.secondary, cursor: "pointer" },
  filterRow: { display: "flex", gap: 6, marginBottom: 12 },
  filterBtn: { padding: "4px 12px", fontSize: 11, fontWeight: 500, border: `1px solid ${T.bg.border}`, borderRadius: 20, background: "transparent", color: T.text.secondary, cursor: "pointer" },
  ...
  txType: { fontSize: 11, color: T.text.secondary },
